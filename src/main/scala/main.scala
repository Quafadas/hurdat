...
@main def acquireData() =
    ...
    // val html = Source.fromURL("https://www.aoml.noaa.gov/hrd/hurdat/hurdat2.html")
    ...
    val allData = (hurricaneSections zip headerLines zip cleanedLines) map { 
    case ((a,b), c) => (a,b,c)
    }
    ...
    TrackInfo(year = year, month = month, day = day, hour=hour, minute = minute, 
    recordType = recordType, systemStatus = status, longditude=longditude, latitude=latitude, latHemisphere = latHem, longHemisphere = longHem,
    maxSustainedWind = maxSustainedWind, minPressure = cp )
    }
    ...
    val allTC = allData.filter(_._2 == 1).map{case(section, header, data) => (section, toTC(data))}
    val tracks = allData.groupBy(_._1).view.mapValues(list => {list.drop(1).map{case(a,b,track) => entryToTrack(track)}}).view
    ...
    val finalTCDb: Seq[HurdatSystem] = allTC.map{
        case(tempId, hurricane) => {
