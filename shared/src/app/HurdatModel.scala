...
object TrackInfo {
  implicit val fooDecoder: Decoder[TrackInfo] = deriveDecoder
  implicit val fooEncoder: Encoder[TrackInfo] = deriveEncoder
}
@JSExportAll
case class HurdatSystem(
    ...
    year: Int = 0,
    name: String = "",
    track: Array[TrackInfo] = Array[TrackInfo]()
) derives ReadWriter

object HurdatSystem {
  implicit val fooDecoder: Decoder[HurdatSystem] = deriveDecoder
  implicit val fooEncoder: Encoder[HurdatSystem] = deriveEncoder
}

case class HurdatSystems(hurricanes: Array[HurdatSystem]) derives ReadWriter
...
def trackColour(trackInfo: TrackInfo) =
  ...
  val code = SystemStatus.valueOf(trackInfo.systemStatus) match
